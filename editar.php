...
<?php 
    ...
    //verifica se o usuario é uma administrador
    ...
    if(isset($_POST["usuario"])){
        ...
        if(!$operacao_alterar) {
            ...
            //se for sucesso redireciona para pagina de administracao
    ...
    if(isset($_GET["codigo"]) && $_GET["codigo"] ==! "" ) {
        ...
        //se nao houver um id redireciona para pagina de administracao
        header("location:add-user.php");   
    }
...
<body class="fundo">
    ...
    <main class="container mt-5 mb-5">
        <div class="principal">
            <!-- inicio do formulario de edicao -->
            <form action="editar.php" method="post">
                <h1 class="text-success" title="Edição de usuário">Edição de usuário</h1>
                <div class="form-group">
                    <label for="exampleInputEmail1">Nome de usuário</label>
                    <input type="text" name="usuario" value="<?php echo $dados_usuario["usuario"]?>" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" placeholder="Nome de usuário">
                </div>
                <div class="form-group">
                    <label for="exampleInputEmail1">Endereço de email</label>
                    <input type="email" value="<?php echo $dados_usuario["email"]?>" name="email" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" placeholder="nome@exemplo.com">
                    <small id="emailHelp" class="form-text text-muted">Nunca vamos compartilhar seu email, com ninguém.</small>
                </div>
                <div class="form-group">
                    <label for="exampleInputPassword1">Senha</label>
                    <input type="text" value="<?php echo $dados_usuario["senha"]?>" name="senha" class="form-control" id="exampleInputPassword1" placeholder="Senha">
                    <input type="hidden" value="<?php echo $dados_usuario["ID"]?>" name="ID">
                </div>
                <button title="Editar" type="submit" class="btn btn-primary btn-lg btn-block">Editar</button>
                <a title="Cancelar edição" href="add-user.php" class="btn btn-danger btn-lg btn-block">Cancelar edição</a>
            </form>
            <!-- final do formulario de edicao -->
        </div>
    </main>
